...
const DraftRoom = () => {
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-neutral-900 text-white z-0 h-[100dvh]">
      ...
      {/* CLOCK */}
      <div className={`flex-none p-4 border-b border-neutral-800 transition-colors duration-500 ${isMyTurn ? 'bg-green-900/30 border-green-800' : 'bg-neutral-800'}`}>
        <div className="flex items-center justify-between">
                ...
                <div>
                    ...
                    {currentPick ? (
                        <div className="font-bold text-lg truncate max-w-[160px]">
                            ...
                            {isMyTurn && <span className="ml-2 text-[10px] bg-green-500 text-black px-1.5 py-0.5 rounded font-black align-middle animate-pulse">YOU</span>}
                        </div>
                    ) : (
            ...
            {isBotTurn && <button onClick={simulateBotPick} disabled={loading} className="bg-blue-600 hover:bg-blue-500 text-xs font-bold px-3 py-2 rounded shadow transition">{loading ? '...' : 'Skip'}</button>}
        </div>
      </div>

      {/* CONTENT */}
      <div className="flex-1 overflow-y-auto bg-neutral-900 relative">
        <div className="p-4 pb-48 min-h-full">
            
            {/* MARKET TAB */}
            {activeTab === 'market' && (
                <div className="animate-in fade-in zoom-in duration-300">
                    <div className="flex bg-neutral-800 p-1 rounded-lg mb-4">
                        <button onClick={() => setMarketFilter('drivers')} className={`flex-1 py-1.5 rounded-md text-xs font-bold transition ${marketFilter === 'drivers' ? 'bg-f1-red text-white shadow' : 'text-gray-400'}`}>Drivers ({availableDrivers.length})</button>
                        <button onClick={() => setMarketFilter('constructors')} className={`flex-1 py-1.5 rounded-md text-xs font-bold transition ${marketFilter === 'constructors' ? 'bg-blue-600 text-white shadow' : 'text-gray-400'}`}>Teams ({availableConstructors.length})</button>
                    </div>

                    {/* DRIVERS LIST */}
                    {marketFilter === 'drivers' && (
                            ...
                            {availableDrivers.map(d => {
                                ...
                                return (
                                    <button 
                                        ...
                                        className={`
                                            flex items-center gap-3 p-3 rounded-xl border text-left shadow-lg transition-transform active:scale-95 relative overflow-hidden
                                            ${!isMyTurn 
                                                ? 'opacity-60 border-neutral-800' // REMOVED 'grayscale', Increased Opacity to 60
                                                : 'hover:scale-[1.02] border-neutral-700/50'
                                            }
                                        `}
                                        style={{ 
                                    ...
                                    >
                                        <div className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-lg shadow-inner z-10 border border-white/10">
                                            🏎️
                                        </div>
                                        ...
                                        {isMyTurn && <div className="ml-auto text-white/80 font-black text-xl z-10 drop-shadow-md">➜</div>}
                                    </button>
                                )
                            })}
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {availableConstructors.map(c => (
                                <button 
                                    ...
                                    `}
                                    style={{ 
                                        background: getTeamGradient(c.name)
                                    }}
                                >
                                    <div className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-lg shadow-inner z-10 border border-white/10">
                                        🔧
            ...
            {activeTab === 'board' && (
                <div className="animate-in slide-in-from-right duration-300 space-y-3">
                    <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-2">Draft Board</h2>
                    {allRosters.map(team => (
                        <div key={team.id} className="bg-neutral-800 p-3 rounded-xl border border-neutral-700">
                            <div className="flex justify-between mb-2 pb-2 border-b border-neutral-700">
                            ...
                            <div className="flex gap-1">
                                {[0,1,2].map(i => (
                                    <div key={i} className={`flex-1 h-7 rounded flex items-center justify-center text-[10px] font-bold border ${team.drivers[i] ? 'bg-neutral-700 border-red-900 text-white' : 'bg-neutral-900 border-neutral-800 text-gray-600'}`}>
                                        {team.drivers[i] ? getAbbr(team.drivers[i]) : ''}
                                    </div>
                                ))}
                                <div className={`w-1/4 h-7 rounded flex items-center justify-center text-[10px] font-bold border ${team.constructor ? 'bg-blue-900/20 border-blue-800 text-blue-200' : 'bg-neutral-900 border-neutral-800 text-gray-600'}`}>
                                    {team.constructor ? getAbbr(team.constructor) : 'tm'}
                                </div>
                            </div>
                        </div>
                    ))}
      ...
      {/* NAV TOGGLE */}
      <div className="flex-none fixed bottom-24 left-6 right-6 z-40 md:hidden">
         <div className="flex p-1 rounded-2xl overflow-hidden border border-white/10 shadow-2xl backdrop-blur-xl backdrop-saturate-150 bg-black/40">
            <button 
                ...
                className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest transition-all duration-300 rounded-xl ${activeTab === 'market' ? 'bg-white/20 text-white shadow-inner border border-white/10' : 'bg-transparent text-white/40 hover:text-white'} `}
            >
                Market
            </button>
            <button 
                onClick={() => setActiveTab('board')} 
                className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest transition-all duration-300 rounded-xl ${activeTab === 'board' ? 'bg-white/20 text-white shadow-inner border border-white/10' : 'bg-transparent text-white/40 hover:text-white'} `}
            >
                Board
            </button>
         </div>
