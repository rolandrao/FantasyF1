...
const MyTeam = () => {
  ...
  const [recaps, setRecaps] = useState([]) // <--- NEW: List of past races
  ...
  // --- MODAL STATE ---
  const [modalType, setModalType] = useState(null) // 'stats' or 'recap'
      ...
      if (myTeam) {
        ...
        const { data: picks } = await supabase
            ...
            .select(`
                ...
                drivers!draft_picks_driver_id_fkey (id, name, constructors (name)),
  ...
  // --- STATS MODAL LOGIC (Drivers/Constructors) ---
  ...
  // --- RECAP MODAL LOGIC (Race Details) ---
  const openRecapModal = async (race) => {
    ...
    // 1. Get my Driver Results for this race
    ...
    // 2. Get my Constructor Results (The 2 drivers belonging to my constructor)
    ...
    <div className="min-h-screen bg-neutral-900 text-white pb-24 md:pb-10 relative">
      ...
      {modalType && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={closeModal}>
            <div className="bg-neutral-800 w-full max-w-md rounded-2xl border border-neutral-600 shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
                ...
                {/* Close Button */}
                <button onClick={closeModal} className="absolute top-4 right-4 text-gray-500 hover:text-white z-10 text-xl">✕</button>

                {/* --- CONTENT A: STATS MODAL --- */}
                {modalType === 'stats' && (
                    <>
                        <div className="bg-neutral-900 p-6 text-center border-b border-neutral-700">
                            <div className="text-4xl mb-2">{selectedItem.type === 'driver' ? '🏎️' : '🔧'}</div>
                            <h2 className="text-2xl font-black italic">{selectedItem.type === 'driver' ? selectedItem.drivers.name : selectedItem.constructors.name}</h2>
                            <p className="text-f1-red font-bold uppercase tracking-widest text-xs">Season Stats</p>
                        </div>
                ...
                {modalType === 'recap' && (
                    <>
                        <div className="bg-neutral-900 p-6 text-center border-b border-neutral-700">
                            <h2 className="text-2xl font-black italic">{selectedItem.race_name}</h2>
                            <p className="text-green-500 font-bold text-lg">{selectedItem.total_points} pts</p>
                        </div>
      ...
      <div className="bg-neutral-800 border-b border-neutral-700 p-6 md:p-10">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-8 text-center md:text-left">
            <div className="w-20 h-20 md:w-24 md:h-24 bg-gradient-to-br from-f1-red to-red-900 rounded-full flex items-center justify-center text-3xl md:text-4xl shadow-xl border-4 border-neutral-700">🧢</div>
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
        ...
        {/* DRIVERS & CONSTRUCTORS (Existing Code) */}
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {drivers.map((pick) => (
                    <div key={pick.pick_number} onClick={() => openStatsModal(pick, 'driver')} className="bg-neutral-800 p-4 rounded-xl border border-neutral-700 relative overflow-hidden group hover:border-f1-red hover:bg-neutral-750 transition cursor-pointer">
                        <div className="absolute top-0 right-0 bg-neutral-900 text-gray-500 text-xs px-2 py-1 rounded-bl">Pick #{pick.pick_number}</div>
                        <div className="mt-2">
                            <div className="text-2xl font-bold">{pick.drivers.name}</div>
                            <div className="text-sm text-gray-400">{pick.drivers.constructors?.name || 'Free Agent'}</div>
                        </div>
        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {constructorPick ? (
                    <div onClick={() => openStatsModal(constructorPick, 'constructor')} className="bg-gradient-to-r from-neutral-800 to-neutral-850 p-6 rounded-xl border border-blue-900/50 relative overflow-hidden shadow-lg cursor-pointer hover:border-blue-500 transition">
                        <div className="absolute top-0 right-0 bg-blue-900/50 text-blue-200 text-xs px-3 py-1 rounded-bl font-bold">Constructor</div>
                        <div className="flex items-center gap-4">
                            <div className="text-4xl">🏁</div>
                            <div>
                                <div className="text-2xl font-black italic">{constructorPick.constructors.name}</div>
                                <div className="text-sm text-gray-400">Click for Stats</div>
                            </div>
                        </div>
                    </div>
                ) : (
